...
void dump_addrinfo(const struct addrinfo *ai)
{
	...
	LOG_DBG(
	    ...
	    ai->ai_addr->sa_family, sock_addr->sin_port,
	    sock_addr->sin_addr.s4_addr[0], sock_addr->sin_addr.s4_addr[1],
	    sock_addr->sin_addr.s4_addr[2], sock_addr->sin_addr.s4_addr[3]);
}
#endif
...
int resource_acquire(struct thread_safe_resource *resource,
    resource_acquired_cb_t cb, k_timeout_t timeout)
{
	int err = 0;

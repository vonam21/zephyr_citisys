...
/* We won't allow two of the same connection to connect to the same host/port
 * etc This will starve the connection of other possible connections. */
static int is_already_connected(const struct mqtt_connection_params *params)
{
	for (int i = 0; i < CONFIG_LIBS_MQTT_MAX_CLIENT_SUPPORT; i++) {
		...
		struct mqtt_connection_params *this_client_params =
		    &runtime.client[i].params.connection_params;
		if (strncmp(params->host, this_client_params->host,
		        strlen(this_client_params->host))) {
			continue;
		}

		if (strncmp(params->port, this_client_params->port,
		        strlen(this_client_params->port))) {
			continue;
		}

		if (strncmp(params->client_id, this_client_params->client_id,
		        strlen(this_client_params->client_id))) {
			continue;
		}

		if (strncmp(params->user_name, this_client_params->user_name,
		        strlen(this_client_params->user_name))) {
			continue;
		}
    ...
    struct mqtt_client *const client, const struct mqtt_evt *evt)
{
	...
	switch (evt->type) {
		case MQTT_EVT_CONNACK:
			if (evt->result != 0) {
				LOG_ERR("client %d: MQTT connect failed %d",
				    client_fd, evt->result);
				break;
			}
		...
		case MQTT_EVT_DISCONNECT:
			LOG_DBG("client %d: MQTT client disconnected %d",
			    client_fd, evt->result);

			if (params->callbacks.on_disconnect) {
				params->callbacks.on_disconnect(
				    client, evt->result);
			}
		...
		case MQTT_EVT_PUBLISH:
			...
			libs_client->puback.message_id =
			    evt->param.publish.message_id;
			mqtt_publish_qos1_ack(
			    &libs_client->context, &libs_client->puback);
			break;

		case MQTT_EVT_PUBACK:
			if (evt->result != 0) {
				LOG_ERR("client %d: MQTT PUBACK error %d",
				    client_fd, evt->result);
				break;
			}
			break;
		...
		case MQTT_EVT_PUBREC:
			if (evt->result != 0) {
				LOG_ERR("client %d: MQTT PUBREC error %d",
				    client_fd, evt->result);
				break;
			}

			LOG_INF("client %d: PUBREC packet id: %u", client_fd,
			...
			if (err != 0) {
				LOG_ERR(
				    "client %d: Failed to send MQTT PUBREL: %d",
				    client_fd, err);
			}
			break;
		...
		case MQTT_EVT_PUBCOMP:
			if (evt->result != 0) {
				LOG_ERR("client %d: MQTT PUBCOMP error %d",
				    client_fd, evt->result);
				break;
			}
...
{
	int err;
	struct sockaddr_in *broker_ipv4 =
	    (struct sockaddr_in *)&libs_client->broker;
	...
	if (err) {
		LOG_DBG("modem_getaddrinfo %s status: %d",
		    libs_client->params.connection_params.host, err);
		return err;
	}
	...
	dump_addrinfo(res);
#endif
	(void)memcpy(broker_ipv4, ((struct sockaddr_in *)res->ai_addr),
...
static void client_init(struct libs_mqtt_client *libs_client)
{
	...
	struct mqtt_connection_params *connection_params =
	    &libs_client->params.connection_params;
...
static int process_mqtt_and_sleep(int client_fd, int timeout)
{
	...
	if (libs_client->connected) {
		if (wait(client_fd, timeout)) {
			...
			if (err) {
				/* This is just to stop the library from
				 * complaining about not having any data */
				if (err != -EAGAIN) {
					LOG_ERR("client %d: mqtt_input err %d",
					    client_fd, err);
				}
				return err;
			}
...
int libs_mqtt_connect(const struct mqtt_client_params *params)
{
	...
	struct mqtt_connection_params *connection_params =
	    &libs_client->params.connection_params;
	(void)memcpy(
	    &libs_client->params, params, sizeof(struct mqtt_client_params));
	...
	libs_client->will_topic.topic.size =
	    strlen(connection_params->will_topic);
	libs_client->will_message.size =
	    strlen(connection_params->will_message);
...
int libs_mqtt_init()
{
	...
	k_work_reschedule(
	    &runtime.mqtt_process_work, K_MSEC(MQTT_PROCESS_TIMEOUT_MSECS));
